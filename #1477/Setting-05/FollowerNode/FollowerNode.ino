...
#define MY_PARENT_NODE_ID 30
// Disable fall back if parent node fails
#define MY_PARENT_NODE_IS_STATIC
// Disable node to node transport feature
...
void receive(const MyMessage& message)
{
	...
	if (message.type == V_STATUS) {
		if (message.sender == LEADER_NODE_ID && message.isEcho()) {
...
bool sendMessage()
{
	...
	msg.setDestination(LEADER_NODE_ID);

	// Pin debug: send message
	digitalWrite(DEBUG_PIN2, HIGH);
	bool success = send(msg, true);
	digitalWrite(DEBUG_PIN2, LOW);
